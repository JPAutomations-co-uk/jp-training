...
drop table if exists standard_channels cascade;
drop table if exists channels cascade;
drop table if exists community_channels cascade; -- old pre-rename name, in case it still lingers

create table standard_channels (
...
create index standard_channels_position_idx on standard_channels(position);

alter table standard_channels enable row level security;
...
create policy "Approved members can read channels"
  on standard_channels for select
  using (
    is_admin() or exists (
  ...
  with check (
    ...
    and (
      ...
      or (
        ...
        and not exists (select 1 from standard_channels c where c.id = channel_id and c.admin_only_post = true)
      )
    )
  );
